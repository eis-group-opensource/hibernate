...
import org.hibernate.action.OrphanRemovalAction;
...
public class ActionQueue {
	...
	private ArrayList orphanRemovals;
	...
	private void init() {
		...
		orphanRemovals = new ArrayList( INIT_QUEUE_LIST_SIZE );
	}
	...
	public boolean areInsertionsOrDeletionsQueued() {
		return ( insertions.size() > 0 || deletions.size() > 0 || orphanRemovals.size() > 0);
	}
	...
	public void serialize(ObjectOutputStream oos) throws IOException {
		...
		log.trace( "starting serialization of [" + queueSize + "] orphanRemovals entries" );
		oos.writeInt( queueSize );
		for ( int i = 0; i < queueSize; i++ ) {
	...
	public static ActionQueue deserialize(
		...
		queueSize = ois.readInt();
		log.trace( "starting deserialization of [" + queueSize + "] orphanRemoval entries" );
		rtn.orphanRemovals = new ArrayList( queueSize );
		for ( int i = 0; i < queueSize; i++ ) {
